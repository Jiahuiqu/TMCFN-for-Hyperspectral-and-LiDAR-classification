...
class GNN_encoder(torch.nn.Module):
    def __init__(self, num_node_features, num_classes):
        ...
        self.conv1 = SAGEConv(num_node_features, num_classes)
...
class Text_Guided_Attention_Feature_Fusion_Module(torch.nn.Module):
    ...
    def forward(self, x1, x2, x3, ft):
        ...
        e1 = self.MLP_Image(x1) + self.MLP_Text(ft_weighted)
        e2 = self.MLP_Image(x2) + self.MLP_Text(ft_weighted)
        e3 = self.MLP_Image(x3) + self.MLP_Text(ft_weighted)
        image_weights = torch.nn.functional.softmax(torch.cat([e1, e2, e3], dim=1), dim=1)
...
class Vision_Net_Final(nn.Module):
    def __init__(self, hparams, patchsize_HSI, device1, device2, class_num):
        ...
        self.Graph_DGFL = DGM_Model(hparams).to(self.device2)
...
class Model_All(torch.nn.Module):
    def __init__(self, hparams,  patchsize_HSI, device1, device2, class_num):
        ...
        self.text_net = Text_Net(embed_dim=512,
                                 ...
                                 transformer_heads=8,
                                 transformer_layers=12).to(self.device2)

    def forward(self, patch_hsi, patch_lidar, labeled_mask, text, warm_up, len_labeled=None):

        with torch.no_grad():
            text = clip.tokenize(text).to(self.device2)
        ...
        Z_vison,loss_vision_CL,logprobs = self.vision_net(patch_hsi=torch.squeeze(patch_hsi).to(self.device1),
                                    patch_lidar=torch.squeeze(patch_lidar).to(self.device1), ft=Z_text,
                                    labeled_mask=labeled_mask, warm_up=warm_up, len_labeled=len_labeled)
